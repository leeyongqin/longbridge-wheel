...
从 thetagang 移植，主要变更：
- RuntimeConfig 移除 ibc / watchdog / ib_async，新增 longbridge: LongbridgeConfig
- Config 对应属性同步更新
- 其余策略逻辑、验证规则、辅助方法保持不变
"""
...
from longbridge_wheel.config_models import (
    AccountConfig,
...
class RuntimeConfig(BaseModel):
    ...
    与 thetagang 的主要差异：
    - 替换 ibc / watchdog / ib_async → longbridge（Longbridge API 专属配置）
    """
...
class Config(BaseModel, DisplayMixin):
    ...
    def apply_strategy_overrides(self) -> "Config":
        ...
        if (
            ...
            self.strategies.wheel.defaults.write_when.calls.min_threshold_percent_relative = (
                wheel_defaults.write_calls_only_min_threshold_percent_relative
            )
    ...
    @property
    def longbridge(self) -> LongbridgeConfig:
        """Longbridge API 配置（替代原 ibc / watchdog / ib_async）"""
        return self.runtime.longbridge

    # --- 快捷属性：从 portfolio / strategies 段访问 ---
...
DEFAULT_RUN_STRATEGIES: list[str] = ["wheel", "cash_management"]
# 注意：vix_call_hedge 在 v1 中禁用，不加入默认策略列表
